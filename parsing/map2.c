...
** Last update Fri Jan 18 22:37:32 2013 thomas leclercq
*/
...
t_block		*generate_block(char *str, int y, int x, t_infos *infos)
{
  ...
  if (blck->str[0] == 'S')
    {
      infos->p_infos->y = blck->y_tp * B_SIZE + (B_SIZE / 2);
      infos->p_infos->x = blck->x_tp * B_SIZE + (B_SIZE / 2);
      infos->p_infos->angle = blck->angle_tp;
      blck->wall = '0';
    }
  return (blck);
}
